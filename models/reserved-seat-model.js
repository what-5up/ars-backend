 ...
 */
async function addReservedSeats(accType, reservedSeats, bookingID, scheduledFlightID, connection = null) {
	return new Promise((resolve, reject) => {
		...
		const result = connection.query(query, values, function (error, results) {
			if (error) {
 ...
 * @returns {Promise<object>} Promise of a query output
 */
async function deleteReservedSeats(accType, booking_id, connection = null) {
	return new Promise((resolve, reject) => {
		...
		const result = connection.query(
			'DELETE FROM reserved_seat WHERE booking_id = ?;',
			[booking_id],
